...
{
    $bookshelves = Bookshelf::all(); // Ambil semua rak buku
    $query = Book::query()->with('bookshelf'); // Query buku dengan relasi bookshelf

    // Filter berdasarkan rak jika dipilih
    if ($request->filled('bookshelf_id')) {
        $query->where('bookshelf_id', $request->bookshelf_id);
    }

    $data['books'] = $query->paginate(5); // Pagination
    $data['bookshelves'] = $bookshelves; // Kirim rak buku ke view
